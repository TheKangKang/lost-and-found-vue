      ...
      <el-container>
        ...
        <el-main>
          ...
          <el-table :data="pageUsers" border style="width: 100%" size="mini"
            ...
            <el-table-column prop="real_name" label="姓名" align="center" width="60">
            </el-table-column>
  ...
  methods: {
    ...
    deleteUsers() {
      this.$confirm('是否确认批量删除' + this.selectUsers.length + '个用户信息', '提示', {
        ...
        let that = this;
        axios
            .post(that.baseURL + 'manager/deletes/', {user: that.selectUsers})
            .then(res => {
              if (res.data.code === 1) {
    ...
    deleteUser(row) {
      ...
      this.$confirm('是否确认删除管理员信息【用户名：' + row.username + '】', '提示', {
        ...
        let that = this;
        axios
            .post(that.baseURL + 'manager/delete/', { username: row.username })
            .then(res => {
              if (res.data.code === 1) {
                //获取所有信息
                that.students = res.data.data;
    ...
    exportExcelGet(){
      let that = this;
      axios
          .get(that.baseURL + 'excel/Mexport/')
          .then(res => {
            if(res.data.code === 1) {
    ...
    uploadExcelPost(file){
      ...
      axios(
          {
            method: 'post',
            url: that.baseURL + 'excel/Mimport/',
            data: fileReq,
          }
      ).then(res => {
        if (res.data.code === 1) {
    ...
    queryUsers() {
      ...
      axios
          .post(that.baseURL + "/managers/query/", {
    ...
    getUsers: function () {
      let that = this;
      axios
          .get(this.baseURL + "getAll/manager/")
          .then(function (res) {
            //console.log(res);
            if (res.data.code === 1) {
              that.users = res.data.data;
